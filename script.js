...
let isSoundEnabled = localStorage.getItem("countdown_sound") !== "false";
...
function updateDigit(id, newVal) {
  ...
  inner.style.transition = "transform 0.4s cubic-bezier(0.4, 0.0, 0.2, 1)";
...
function startTimer() {
  ...
  if (isSoundEnabled) {
    if (!alarmSound.src || alarmSound.src === "")
      alarmSound.src = "sounds/timer.mp3";
    alarmSound
      .play()
      .then(() => {
        alarmSound.pause();
  ...
  let lastTickSecond = totalSeconds;
  ...
  const runTick = () => {
    ...
    if (secondsLeft < lastTickSecond) {
      if (isSoundEnabled && secondsLeft > 0) {
    ...
    if (totalSeconds <= 0) {
      ...
      if (isSoundEnabled) {
        if (!alarmSound.src.includes("sounds/timer.mp3"))
          alarmSound.src = "sounds/timer.mp3";
...
["mousemove", "keypress", "scroll", "touchmove"].forEach((evt) =>
...
function setupSoundSwitcher() {
  ...
  function updateIcon() {
    ...
    btn.style.opacity = isSoundEnabled ? "1" : "0.7";
  }
  ...
  btn.addEventListener("click", () => {
    ...
    localStorage.setItem("countdown_sound", isSoundEnabled);
    updateIcon();
  });
}
  ...
  overlay.innerHTML = `
        <div class="onboarding-card">
            ...
            <div class="onboarding-actions">
                <button class="btn-secondary" id="ob-skip"></button>
                <button class="btn-primary" id="ob-next"></button>
            </div>
        </div>
    `;
...
function setupOrientationControls() {
  ...
  rotateBtn.addEventListener("click", () => {
    timerScreen.classList.toggle("force-landscape");
  });
  ...
  function checkOrientation() {
    ...
    if (isLandscape) {
      timerScreen.classList.remove("force-landscape");
